...
class JsonSeedDecoderTest {
    ...
    fun `should decode valid json with correct password`() {
        JSONS.forEachIndexed { index, json ->
            ...
            assertInstance<jp.co.soramitsu.fearless_utils.keyring.model.NetworkTypeIdentifier.Genesis>(networkTypeIdentifier)
    ...
    fun `should extract meta from ethereum json`() {
        ...
        assertEquals(data.encryption.encryptionType, jp.co.soramitsu.fearless_utils.keyring.EncryptionType.ECDSA)
    }
    ...
    fun `should extract meta from valid json`() {
        ...
        assertEquals(jp.co.soramitsu.fearless_utils.keyring.EncryptionType.SR25519, data.encryption.encryptionType)
        assertInstance<jp.co.soramitsu.fearless_utils.keyring.model.NetworkTypeIdentifier.Genesis>(networkTypeIdentifier)
        assertEquals(VALID_GENESIS, networkTypeIdentifier.genesis)
    ...
    fun `should handle json with no genesis`() {
        ...
        assertInstance<jp.co.soramitsu.fearless_utils.keyring.model.NetworkTypeIdentifier.AddressByte>(networkTypeIdentifier)
    ...
    fun `should handle json with no network info`() {
        ...
        assertInstance<jp.co.soramitsu.fearless_utils.keyring.model.NetworkTypeIdentifier.Undefined>(result.networkTypeIdentifier)
    }
