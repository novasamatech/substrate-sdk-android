...
/**
 * Extract Public Key from a raw secret key (soft derivation result).
 *
 * @param secret: [32 bytes private key scalar | 32 bytes nonce]
 * @return public key [32 bytes]
 */
#[no_mangle]
pub unsafe extern "system" fn Java_io_novasama_substrate_1sdk_1android_encrypt_Sr25519_getPublicKeyFromSecret(
    jni_env: JNIEnv,
    _: JClass,
    secret: jbyteArray,
) -> jbyteArray {
    ...
    let secret_key = try_or_throw_null!(jni_env, create_secret(secret_vec.as_slice()));
